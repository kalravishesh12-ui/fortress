"""
MCP-Shield CLI Application (Wrap, Serve, Audit, Simulate, Policy).
"""
...
from mcp_shield.config import AuditLedgerConfig, MCPShieldPolicy, load_policy, save_policy
...
def wrap(
    ...
    policy: Optional[str] = typer.Option(None, "--policy", "-p", help="Path to custom mcp-policy.yaml file"),
):
    """
    Transparently wrap a local MCP server over stdio for Claude Desktop or Cursor.
    """
    if not command:
        console.print("[bold red]Error:[/bold red] No command provided. Usage: mcp-shield wrap -- <command> [args...]")
...
def serve(
    ...
    policy: Optional[str] = typer.Option(None, "--policy", help="Path to mcp-policy.yaml file"),
):
    """
    Run the Remote MCP HTTP+SSE Gateway and Web Dashboard on localhost:9090.
    """
    console.print(Panel(
        f"[bold green]🛡️ MCP-Shield Enterprise Gateway active[/bold green]\n"
        ...
        f"Policy Source:         [yellow]{policy or 'mcp-policy.yaml (default)'}[/yellow]",
...
def verify_audit(
    db: str = typer.Option("./mcp-shield-audit.db", "--db", "-d", help="Path to SQLite audit ledger database"),
...
def init_policy(
    output: str = typer.Option("mcp-policy.yaml", "--output", "-o", help="Target output file for policy template"),
    force: bool = typer.Option(False, "--force", "-f", help="Overwrite existing policy file"),
):
    """
    Generate a default production-grade declarative mcp-policy.yaml file.
    """
...
def test_payload(
    ...
    console.print(Panel(
        ...
        title="[bold white]MCP-Shield Scan Results[/bold white]",
...
def show_version():
    """
    Print MCP-Shield version information.
    """
    console.print(f"[bold cyan]MCP-Shield[/bold cyan] version [bold green]{__version__}[/bold green]")
