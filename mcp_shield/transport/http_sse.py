"""
HTTP + SSE Remote Gateway & Administration Server for MCP-Shield.
"""
...
from mcp_shield.config import MCPShieldPolicy, load_policy
...
def create_gateway_app(policy_path: Optional[Union[str, MCPShieldPolicy]] = None) -> FastAPI:
        ...
        if inbound_res.verdict == SecurityVerdict.BLOCK:
            return JSONResponse(
                ...
                content={
                    ...
                    "error": {
                        "code": -32000,
                        "message": f"Blocked by MCP-Shield: {inbound_res.blocked_reason}",
    ...
    async def get_raw_policy():
        ...
        if os.path.exists("mcp-policy.yaml"):
            with open("mcp-policy.yaml", "r", encoding="utf-8") as f:
                return Response(content=f.read(), media_type="text/yaml")
