"""
Embedded Web Dashboard for MCP-Shield (Real-time Metrics, Audit Ledger, HITL & Controls).
"""
...
<head>
    ...
    <title>MCP-Shield | Enterprise Security Gateway & Agent Firewall</title>
...
<body class="bg-shield-dark text-gray-100 font-sans min-h-screen antialiased" x-data="dashboardApp()" x-init="init()">
    ...
    <main class="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div x-show="activeTab === 'policy'" class="space-y-4" x-cloak>
            <h2 class="text-base font-semibold">Active Declarative Policy (mcp-policy.yaml)</h2>
        ...
        function dashboardApp() {
            return {
                ...
                async verifyLedger() {
                    ...
                    if (res.is_valid) {
                        alert('? Tamper Verification Succeeded! Every block in the hash chain is cryptographically authentic.');
                    } else {
                        alert('? Verification Failed! Inconsistencies detected:
' + res.errors.join('
'));
                    }
                },
