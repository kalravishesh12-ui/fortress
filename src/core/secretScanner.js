    ...
    if (b64Matches) {
      for (const rawB64 of b64Matches) {
        try {
          ...
          // Decoding error or malformed sequence; safe to skip inspecting this token
          if (process.env.FORTRESS_DEBUG) {
            console.error(`[Fortress:SecretScanner] Base64 decode inspection skipped: ${err?.message || err}`);
          }
        }
      }
    }
