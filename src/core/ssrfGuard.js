...
class SSRFGuard {
  ...
  checkString(raw) {
    ...
    try {
      ...
      // Invalid URL format; fall back to raw host/ip extraction
    }
  ...
  async openPinnedConnection(urlString, timeoutMs = 10000) {
    ...
    return new Promise((resolve, reject) => {
      const socket = net.createConnection({
        ...
        if (connectedAddress && connectedAddress !== pinnedIp) {
          socket.destroy();
          reject(new Error(`SSRF DNS Rebinding Detected: Connected to ${connectedAddress} instead of pinned IP ${pinnedIp}`));
          return;
        }
