...
class StdioProxy:
    ...
    def __init__(self, command: List[str], policy: Optional[MCPShieldPolicy] = None):
        ...
        self.session_cumulative_bytes = 0
        ...
        async def handle_inbound():
            while True:
                ...
                try:
                    ...
                    # Pass through non-JSON-RPC lines directly to the wrapped MCP server
            ...
            while True:
                ...
                try:
                    ...
                    if line_len > max_bytes:
                        self.console.print(f"[bold red]🚫 BLOCKED Outbound response: size {line_len} exceeds limit of {max_bytes} bytes[/bold red]")
                        error_resp = {
                            "jsonrpc": "2.0",
                            "id": None,
                            "error": {
                                "code": -32000,
                                "message": f"Blocked by Fortress: Response payload size ({line_len} bytes) exceeds limit ({max_bytes} bytes).",
                            },
                        }
                        self._write_client_stdout(error_resp)
                        continue

                    # Cumulative session response byte tracking
                    self.session_cumulative_bytes += line_len
                    max_session_bytes = getattr(self.engine.policy.outbound_guard, "max_session_bytes", 50 * 1024 * 1024)
                    if self.session_cumulative_bytes > max_session_bytes:
                        self.console.print(f"[bold red]🚫 BLOCKED Outbound response: cumulative session size {self.session_cumulative_bytes} exceeds limit of {max_session_bytes} bytes[/bold red]")
                        error_resp = {
                            "jsonrpc": "2.0",
                            "id": None,
                            "error": {
                                "code": -32000,
                                "message": f"Blocked by Fortress: Cumulative session response payload ({self.session_cumulative_bytes} bytes) exceeds limit ({max_session_bytes} bytes).",
                            },
                        }
                        self._write_client_stdout(error_resp)
                        continue
                    ...
                    # Pass through non-JSON-RPC lines directly to client
