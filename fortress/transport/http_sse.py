...
class PinSchemaRequest(BaseModel):
    tool_name: str
    description: str = ""
...
def create_gateway_app(policy_path: Optional[Union[str, MCPShieldPolicy]] = None) -> FastAPI:
    ...
    async def mcp_message_handler(request: Request):
        ...
        context = SecurityContext(
            ...
            user_id="sse_agent",
            role=engine.policy.rbac.default_role,
            tenant_id=tenant_id,
    ...
    async def proxy_tool_call(req: ToolCallProxyRequest, request: Request):
        ...
        response_headers = {"X-Tenant-ID": tenant_id} if tenant_id else {}
        return JSONResponse(
            content={
                "status": "allowed",
                "tool": req.tool,
    ...
    async def get_audit_logs(
        ...
        logs = engine.audit_ledger.get_recent_entries(
            limit=limit,
            offset=offset,
            tool_name=tool_name,
            verdict=verdict,
    ...
    async def prune_audit_logs(days: int = 90):
        deleted = engine.audit_ledger.prune_old_entries(days=days)
        return {"status": "pruned", "deleted_entries": deleted, "retention_days": days}

    @app.get("/api/v1/audit/export/stix")
    async def export_stix_bundle(limit: int = 100):
        logs = engine.audit_ledger.get_recent_entries(limit=limit)
        stix_bundle = engine.audit_ledger.export_stix_bundle(logs)
        return Response(content=stix_bundle, media_type="application/json")
    ...
    @app.post("/api/v1/schemas/pin")
    async def pin_schema(action: PinSchemaRequest):
        """Pin and sign a tool schema dynamically."""
        tool_def = {
            "name": action.tool_name,
            "description": action.description,
            "inputSchema": action.input_schema,
        }
        schema_hash = engine.schema_pinner.pin_tool(tool_def)
        return {
            "status": "pinned",
