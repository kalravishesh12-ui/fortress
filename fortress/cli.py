...
def serve(
    ...
    """
    Run the Remote MCP HTTP+SSE Gateway and Web Dashboard (Enterprise Feature).
    """
    create_app_fn = None
    try:
        from fortress_enterprise.gateway.server import create_enterprise_gateway_app
        create_app_fn = create_enterprise_gateway_app
    except ImportError:
        try:
            from fortress.transport.http_sse import create_gateway_app
            create_app_fn = create_gateway_app
        except ImportError:
            pass

    if create_app_fn is None:
        console.print(Panel(
            "[bold yellow]🏢 Centralized Gateway Daemon ('serve') is part of Fortress Enterprise[/bold yellow]\n\n"
            "The open-core distribution provides local developer protection via [bold cyan]fortress wrap[/bold cyan].\n\n"
            "[bold white]Fortress Enterprise Features:[/bold white]\n"
            " • Centralized multi-tenant gateway daemon for VPC-wide agent traffic\n"
            " • Centralized team dashboard & HITL interactive Slack/web approval hooks\n"
            " • Enterprise Identity: Okta, Microsoft Entra ID (Azure AD), SAML 2.0 & SCIM 2.0\n"
            " • SIEM Streaming: Splunk HEC, Datadog Logs API, STIX 2.0, and SOC 2 / DORA compliance reports\n\n"
            "Learn more: [bold cyan]https://fortress-mcp.org/enterprise[/bold cyan]\n"
            "Install:    [bold green]pip install fortress-enterprise[/bold green]",
    ...
    try:
        ...
        console.print("[bold red]Uvicorn is required to run the gateway server. Install with: pip install uvicorn[/bold red]")
        raise typer.Exit(code=1)
    ...
    fastapi_app = create_app_fn(policy)
