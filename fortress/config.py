"""
Declarative Policy Configuration & Validation for Fortress.
"""
...
class GlobalRateLimitConfig(BaseModel):
    enabled: bool = True
    global_requests_per_second: int = 500
    global_burst_allowance: int = 100
    threat_backoff_base: float = 2.0
    max_threat_backoff_seconds: float = 300.0
...
def _load_api_tokens_from_env() -> Dict[str, str]:
    env_val = os.getenv("FORTRESS_API_TOKENS") or os.getenv("MCP_TOKENS")
    if env_val:
        try:
            ...
            for item in env_val.split(","):
                if ":" in item:
                    tok, role = item.strip().split(":", 1)
                    tokens[tok.strip()] = role.strip()
            if tokens:
                return tokens
    return {}


def _default_api_tokens() -> Dict[str, str]:
    env_tokens = _load_api_tokens_from_env()
    if env_tokens:
        return env_tokens

    if os.getenv("FORTRESS_ENV") == "production" or os.getenv("NODE_ENV") == "production":
        warnings.warn(
            "Running in production with default API tokens. Please set FORTRESS_API_TOKENS in your environment.",
            UserWarning,
            stacklevel=2,
        )
...
class RBACConfig(BaseModel):
    ...
    roles: Dict[str, RBACRole] = Field(default_factory=lambda: {
        ...
        "readonly": RBACRole(
            ...
            denied_tools=["*shell*", "*exec*", "*eval*", "*system*", "write_*", "create_*", "delete_*", "update_*", "drop_*"],
            require_approval=[]
        ),
        "agent": RBACRole(
    ...
    api_tokens: Dict[str, str] = Field(default_factory=_default_api_tokens)

    @model_validator(mode="after")
    def _override_env_tokens(self) -> RBACConfig:
        env_tokens = _load_api_tokens_from_env()
        if env_tokens:
            self.api_tokens.update(env_tokens)
        return self
...
class AuditLedgerConfig(BaseModel):
    enabled: bool = True
    db_path: str = "./fortress-audit.db"
    hmac_secret_key: str = Field(
        default_factory=lambda: os.getenv("FORTRESS_HMAC_SECRET", "fortress_deterministic_enterprise_hmac_secret_2026")
    )
...
class MCPShieldPolicy(BaseModel):
    ...
    global_rate_limit: GlobalRateLimitConfig = Field(default_factory=GlobalRateLimitConfig)
...
def load_policy(path: Optional[str] = None) -> MCPShieldPolicy:
    ...
    # Fallback to local fortress-policy.yaml if present
    default_file = "fortress-policy.yaml"
...
def save_policy(policy: MCPShieldPolicy, path: str = "fortress-policy.yaml") -> None:
